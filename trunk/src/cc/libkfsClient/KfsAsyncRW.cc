...
Asyncer::Timeout()
{
    ...
    while (1) {
        /*
        if (mNumRunning >= MAX_CONCURRENT_PREFETCHES)
            return;
        */
...
AsyncWorker::Done()
{
    ...
    mReq->done = true;
    KFS_LOG_VA_INFO(" <- Async done: server = %s, fd=%d, chunk=%ld, offset=%ld, nbytes=%d, res=%d",
...
AsyncWorker::GotHeader(IOBuffer *buffer, int *hdrLen, KfsOp *op)
{
    ...
    *hdrLen = idx + 4;
    ...
    if (seq != op->seq) {
        KFS_LOG_VA_INFO("Seq # mismatch: expect = %d, got = %d", op->seq, seq);
        op->status = -1;
        return true;
    }
    ...
    if (op->status < 0) {
        KFS_LOG_VA_INFO("Async worker failure: bogus response for %s", 
                        op->Show().c_str());
...
{
    KFS_LOG_VA_INFO("+> Async read: seq = %d fd = %d (%s->%s), creating worker for chunk=%ld, offset=%ld, numbytes=%d",
        ...
        case EVENT_NET_READ:
            if (GotHeader(buffer, &headerLen, &mReadOp)) {
                if (mReadOp.status < 0) {
                    ...
                    KFS_LOG_VA_INFO("Async read failure: got back status: %d", 
                                    mReadOp.status);                    
                    Done();
                    break;
                }
