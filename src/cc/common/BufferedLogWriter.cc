...
public:
    Impl(
        ...
        : mMutex(),
          ...
          mTruncatedSuffix(inTrucatedSuffixPtr ? mTruncatedSuffix : "..."),
        ...
        va_list     inArgs)
    {
        ...
        const int  kAvgRecSize        = 256;
        Time       theTimeWaited      = 0;
        for (int i = 0; ; i++) {
            if ((i > 0 || mCurPtr + kAvgRecSize >= mEndPtr) && ! FlushSelf()) {
            ...
            const size_t theMaxMsgLen = theMaxLen - theLen; // with \0
            const int    theRet       = ::vsnprintf(
            ...
            if (mFd >= 0 && ! mFileName.empty()) {
                if (mMaxLogFiles > 0) {
                    if ((int64_t)mCurWritten > mMaxLogFileSize) {
                        ...
                        QCStMutexUnlocker theUnlocker(mMutex);
                        ::close(theFd);
                        RotateLogs(theFileName, theMaxLogFiles);
                    }
...
private:
    ...
    typedef std::vector<string> LogFileNames;
        ...
        int    inKeepCount)
    {
        for (int i = inKeepCount - 1; i >= 0; i--) {
            ::rename(
                (i == 0 ? inFileName : MakeName(inFileName, i)).c_str(),
                MakeName(inFileName, i + 1).c_str()
            );
        }
    }
    ...
    void OpenLogFile()
    {
        ...
        for (LogFileNames::const_iterator theIt = theLogFileNames.begin(); ;) {
            while ((theFd = ::open(
                    theFileName.c_str(), theOpenFlags, theOpenMode)) < 0) {
                theError = errno;
