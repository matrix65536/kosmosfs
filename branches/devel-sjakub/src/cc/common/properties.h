...
class Properties {
  ...
  public  :
    ...
    const char* getValue(const std::string & key, const char* def) const;
    int getValue(const std::string & key, int def);
    long getValue(const std::string & key, long def);
    long long getValue(const std::string & key, long long def);
    uint64_t getValue(const std::string & key, uint64_t def);
