...
public class KosmosFileSystem extends FileSystem {
    ...
    public void initialize(URI uri, Configuration conf) throws IOException {
        ...
        try {
            if (uri.getHost() == null) {
                ...
                kfsImpl = new KFSImpl(uri.getHost(), uri.getPort());
            }
	...
	FileStatus[] entries = listStatus(srep);
	...
	Path absolute = makeAbsolute(path);
        ...
        if (kfsImpl.isDirectory(srep)) {
            ...
            return new FileStatus(0, true, 1, 0, kfsImpl.getModificationTime(srep), path);
        } else {
